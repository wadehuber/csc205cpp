...
/**
 * A simplified linked list, written for CSC205 as an example of building an
 * abstract data type from scratch (a different, simpler implementation than the
 * one in the jsjf package).
 *
 ...
 * same message.
 *
 * This is a header-only template (the Java generic class LinkedList<T>).
 */
template <typename T>
class LinkedList {
private:
    // Java had a private inner class ListNode<E>. In C++ we make it a private
    // nested struct. The list is its only friend/owner.
    struct ListNode {
        T element;
        ListNode* next;
...
public:
    ...
    T removeFirst() {
        ...
        // (Java just nulled the fields; C++ must delete to avoid a leak.)
    ...
    T removeLast() {
        ...
        if (size_ == 1) {
            ...
            //  next = tail
            ListNode* newTail = head;  // Start looking at the tail
    ...
    // ----- Iterator support -----
    // Java exposed iterator() returning an Iterator<T>. In C++ the equivalent
    // of "for (String s : myList)" is a range-based for loop, which needs
    // begin()/end() returning an iterator with operator*, operator++, and
