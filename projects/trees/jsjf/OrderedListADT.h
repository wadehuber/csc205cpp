...
//
// Java's "interface OrderedListADT<T> extends ListADT<T>" becomes a C++ class
// template that publicly inherits from ListADT<T>.
//
// We inherit VIRTUALLY from ListADT so that a concrete list which derives from
// both a *List base and this ADT shares a single ListADT subobject -- this is
// the C++ way to model Java's multiple-interface inheritance (the "diamond")
// without ambiguity.
template <typename T>
class OrderedListADT : public virtual ListADT<T> {
public:
    virtual ~OrderedListADT() = default;
