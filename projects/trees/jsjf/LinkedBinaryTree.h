...
//
// OWNERSHIP NOTE: getLeft()/getRight() build temporary LinkedBinaryTree "views"
// whose root points INTO an existing tree (the nodes are shared, exactly as in
// Java). Such a view must NOT delete those shared nodes, or we would
// double-free. We track this with 'ownsNodes': a normal tree owns its nodes; a
// view does not. (Java did not need this because the GC only frees nodes when
...
public:
    ...
    // C++ needs an explicit destructor (Java's GC did this). Deletes the whole
    // tree unless this object is only a non-owning view.
...
public:
    ...
    LinkedBinaryTree<T> getLeft() {
        if (root == nullptr)
            throw EmptyCollectionException("Get left operation "
    ...
    LinkedBinaryTree<T> getRight() {
        if (root == nullptr)
            throw EmptyCollectionException("Get right operation "
                    "failed. The tree is empty.");
...
public:
    ...
    // ------------------------------------------------------------------
    // Traversals. Java returned a java.util.Iterator over a temp list; C++
    // has no universal iterator type, so each iteratorXxxOrder() builds the
    // temp list (in the proper order) and returns it by value. The driver
    // walks it with a range-based for loop. ArrayUnorderedList provides
    // begin()/end(), so   for (T e : t.iteratorInOrder())   works.
    // ------------------------------------------------------------------
    ArrayUnorderedList<T> iteratorInOrder() {
        ArrayUnorderedList<T> tempList;
        inOrder(root, tempList);
        return tempList;
    }

    ArrayUnorderedList<T> iteratorPreOrder() {
        ArrayUnorderedList<T> tempList;
        preOrder(root, tempList);
        return tempList;
    }

    ArrayUnorderedList<T> iteratorPostOrder() {
        ArrayUnorderedList<T> tempList;
