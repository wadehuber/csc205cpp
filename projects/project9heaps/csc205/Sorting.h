...
public:
    ...
    // ---- bstSort ----
    // Project 8 - STUDENT STUB: the Java starter left this method body empty
    // ("// Project 8"), so it does nothing. Kept as a stub here.
    template <typename T, int N>
    static void bstSort(T (&data)[N]) {
        // Project 8
        (void) data;
    }

    // ---- smallest_n ----
    // Project 9 - STUDENT STUB: the Java starter currently returns ALL elements
    // of data (as a java.util.ArrayList); students modify it to return only n
    // elements. We reproduce the stub: it copies every element into a
    // std::vector<T> (C++'s stand-in for ArrayList) and returns it. The 'n'
    // parameter is intentionally unused until the project is completed.
    template <typename T, int N>
    static std::vector<T> smallest_n(T (&data)[N], int n) {
        std::vector<T> ret;

        for (const T& e : data) {
            ret.push_back(e);   // Java: ret.add(e);
        }
        (void) n;
        return ret;
    }
